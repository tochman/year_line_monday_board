...
class ConfigValidator {
  ...
  static validateItems(items) {
    ...
    return items.map((item, index) => {
      ...
      // Validate dates - startDate is required, endDate is optional (single-day items)
      const startDate = this.validateDate(item.startDate);
      const endDate = item.endDate ? this.validateDate(item.endDate) : null;

      if (!startDate) {
        console.warn(`Item ${item.id} has invalid start date, skipping`);
        return null;
      }
      
      // Use startDate as endDate for single-day items
      ...
      return {
        ...
        // Preserve original dates for cross-year items
        _originalStartDate: item._originalStartDate || null,
        _originalEndDate: item._originalEndDate || null,
        _mondayData: item._mondayData || null,
      };
    }).filter(Boolean);
  }
