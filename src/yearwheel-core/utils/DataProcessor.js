 ...
 * 
 * Ported from main Year Wheel project: December 2025
 */
...
class DataProcessor {
  ...
  computeStructureHash(structure) {
    ...
    // Include ring_order, array index, and all mutable properties to detect any changes
    const ringIds = structure.rings?.map((r, idx) => 
      `${idx}:${r.id}:${r.visible}:${r.ring_order}:${r.type}:${r.name}:${r.color}`
    ).join(',') || '';
    const activityIds = structure.activityGroups?.map((a, idx) => 
      `${idx}:${a.id}:${a.visible}:${a.name}:${a.color}`
    ).join(',') || '';
    const labelIds = structure.labels?.map(l => `${l.id}:${l.visible}:${l.name}:${l.color}`).join(',') || '';
    
    // CRITICAL FIX: Include item dates, ring assignments, and activity assignments
    // This ensures cache invalidates when items are moved/resized/reassigned
    const itemsHash = structure.items?.map(i => 
      `${i.id}:${i.startDate}:${i.endDate}:${i.ringId}:${i.activityId}`
    ).join(',') || '';
