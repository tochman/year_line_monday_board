...
class InteractionHandler {
  ...
  startActivityDrag(event, itemRegion, dragMode = null) {
    ...
    let logicalEndAngle = itemRegion.endAngle;
    
    // CRITICAL FIX: For cross-year items in move mode, calculate the true end angle
    // based on the original duration, not the clamped display angles
    // This ensures the preview shows the full item length during drag
    if (dragMode === 'move' && freshItem._originalStartDate && freshItem._originalEndDate) {
      const originalStartDate = new Date(freshItem._originalStartDate);
      const originalEndDate = new Date(freshItem._originalEndDate);
      const durationMs = originalEndDate.getTime() - originalStartDate.getTime();
      const durationDays = durationMs / (1000 * 60 * 60 * 24);
      
      // Convert duration to angle span (365 days = 360 degrees)
      const durationAngle = (durationDays / 365) * 360;
      const durationRadians = LayoutCalculator.degreesToRadians(durationAngle);
      
      // Calculate the true end angle from start + duration
      logicalEndAngle = logicalStartAngle + durationRadians;
      
      // console.log('[InteractionHandler] Cross-year item move - using true duration:', {
      ...
      //   durationDays,
      //   durationAngle,
      //   logicalStartAngle: LayoutCalculator.radiansToDegrees(logicalStartAngle),
      //   logicalEndAngle: LayoutCalculator.radiansToDegrees(logicalEndAngle)
      // });
    }
  ...
  async stopActivityDrag() {
    ...
    // CRITICAL FIX: In move mode, preserve the original duration for cross-year items
    // The displayed item may be clamped to the current year, but we need to maintain
    // the full original duration when moving the item
    if (this.dragState.dragMode === 'move' && originalItem) {
      const originalStartDate = new Date(originalItem._originalStartDate || originalItem.startDate);
      const originalEndDate = new Date(originalItem._originalEndDate || originalItem.endDate);
      ...
      // Apply the original duration to the new start date
      newEndDate = new Date(newStartDate.getTime() + originalDurationMs);
      
      // console.log('[InteractionHandler] MOVE mode - preserving cross-year duration:', {
      //   originalStartDate: originalStartDate.toISOString().split('T')[0],
      //   originalEndDate: originalEndDate.toISOString().split('T')[0],
      //   originalDurationMs,
    ...
    // Use _originalEndDate if available (for cross-year items that were clamped)
    ...
    // Use _originalStartDate if available (for cross-year items that were clamped)
    ...
    // Only clamp start date if it's unreasonably far out (more than 1 year)
    // This allows cross-year dates but prevents crazy values
    const oneYearBefore = new Date(yearStart);
    oneYearBefore.setFullYear(oneYearBefore.getFullYear() - 1);
    if (newStartDate < oneYearBefore) newStartDate = yearStart;
    
    const oneYearAfter = new Date(yearEnd);
    oneYearAfter.setFullYear(oneYearAfter.getFullYear() + 1);
    if (newStartDate > oneYearAfter) newStartDate = yearEnd;
    ...
    // Only clamp end date if unreasonably far out
    const endOneYearBefore = new Date(yearStart);
    endOneYearBefore.setFullYear(endOneYearBefore.getFullYear() - 1);
    if (newEndDate < endOneYearBefore) newEndDate = yearStart;

    let overflowEndDate = null;

    if (newEndDate > yearEnd) {
      // End date is past December 31 - this is cross-year overflow
      ...
      if (
        ...
        // Only clamp if callback was provided (callback handles the cross-year case)
        newEndDate = yearEnd;
      }
      // If no callback, keep the overflow date - allow cross-year dates
    }
    ...
    if (hasChanges) {
      ...
      // Call onActivityUpdate callback (Monday.com integration)
      if (this.options.onActivityUpdate) {
        console.log('[InteractionHandler] 🔔 Calling onActivityUpdate');
        await this.options.onActivityUpdate(updatedItem);
      }
    }
    
    // Call onDragEnd callback (Monday.com integration)
  ...
  handleMouseMove(event) {
    ...
    for (const itemRegion of clickableItems) {
      if (this.isPointInItemRegion(x, y, itemRegion)) {
        ...
        if (resolvedItem) {
          // Check if item overlaps with the current year (not just starts in it)
          const itemStart = new Date(resolvedItem.startDate);
          const itemEnd = new Date(resolvedItem.endDate || resolvedItem.startDate);
          const viewYear = parseInt(this.wheel.year, 10);
          const yearStart = new Date(viewYear, 0, 1);
          const yearEnd = new Date(viewYear, 11, 31);
          const overlapsViewYear = itemStart <= yearEnd && itemEnd >= yearStart;
          const zoomActive = this.wheel.zoomedMonth !== null || this.wheel.zoomedQuarter !== null;

          if ((zoomActive || overlapsViewYear) && !resolvedItem.isCluster) {
  ...
  handleClick(event) {
    ...
    if (this.options.onItemClick) {
      ...
      if (this.wheel.clickableItems) {
        for (const itemRegion of this.wheel.clickableItems) {
          if (this.isPointInItemRegion(x, y, itemRegion)) {
            // If this is a cluster, extract and pass the items array directly
            if (itemRegion.clusterData && itemRegion.clusterData.items) {
              this.options.onItemClick(itemRegion.clusterData.items, {
                x: event.clientX,
                y: event.clientY
              }, event);
              return;
