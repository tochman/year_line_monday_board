...
class YearWheel {
  // Build timestamp: 2025-12-23T12:55:00Z - preserve original dates for cross-year resize
  static BUILD_VERSION = "2025-12-23-v7";
  ...
  constructor(canvas, year, title, colors, size, events, options) {
    ...
    if (errors.length > 0) {
      ...
      console.warn(
        "Available rings:",
        this.wheelStructure.rings?.map((r) => ({ id: r.id, name: r.name }))
      );
      console.warn(
        "Available activityGroups:",
        this.wheelStructure.activityGroups?.map((a) => ({
          id: a.id,
          name: a.name,
        }))
      );
      console.warn(
        "Items:",
        this.wheelStructure.items?.map((i) => ({
          id: i.id,
          name: i.name,
          ringId: i.ringId,
    ...
    // Background color - passed directly (dynamically detected from CSS variable)
    // This supports ALL Monday.com themes: Light, Dark, Hacker, Night, System default, etc.
    this.backgroundColor = options.backgroundColor || "#ffffff";
    
    // Determine if this is a dark theme by checking background luminance
    this.isDarkTheme = this.isColorDark(this.backgroundColor);
    
    // Text color adapts to theme - dark text on light themes, light text on dark themes
    this.textColor = this.isDarkTheme ? "#323338" : "#323338";
    ...
    // Use month names from options if provided, otherwise fallback to English
    ...
    this.rotationAngle = options.initialRotation || 0;
    ...
    // Highlighted item: when set, this item renders with its FULL date range
    // even if it extends beyond the current year (for cross-year items)
  ...
  /**
   * Set the highlighted item (for showing full cross-year span)
   * @param {Object|null} item - Item to highlight, or null to clear
   */
  setHighlightedItem(item) {
    const changed = this.highlightedItem?.id !== item?.id;
    this.highlightedItem = item;
    if (changed) {
      this.create(); // Redraw to show/hide full span
    }
  }

  // Generate cache key to detect when background needs redrawing
  getCacheKey() {
    const ringCount = this.wheelStructure.rings.filter((r) => r.visible).length;
  ...
  updateZoomState(zoomedMonth, zoomedQuarter) {
    ...
    if (monthChanged || quarterChanged) {
      this.zoomedMonth =
        zoomedMonth !== undefined && zoomedMonth !== null ? zoomedMonth : null;
      this.zoomedQuarter =
        zoomedQuarter !== undefined && zoomedQuarter !== null
          ? zoomedQuarter
          : null;
  ...
  updateActiveEditors(newActiveEditors) {
    const changed =
      JSON.stringify(this.activeEditors) !== JSON.stringify(newActiveEditors);
  ...
  collectEditorAvatar(item, startRadius, itemWidth, adjustedEndAngle) {
    ...
    const editor = this.activeEditors.find(
      (e) => e.itemId === item.id && e.activity === "editing"
    );
    if (!editor) {
      return;
    }
  ...
  drawEditorAvatarInRotatedContext(
    item,
    editor,
    startRadius,
    itemWidth,
    ...
    this.context.fillStyle = "#FFFFFF";
    this.context.font = `bold ${Math.floor(
      avatarSize * 0.5
  ...
  checkDragCollision(startDate, endDate, targetRingId, excludeItemId) {
    ...
    // Use DataProcessor for memoized visibility filtering
  ...
  // Check if a color is dark (for theme detection)
  // Returns true if luminance is below 0.5
  ...
  drawRingNameBand(ringName, startRadius, bandWidth) {
    ...
    this.context.fillStyle = bandBackground;
    this.context.fill();
    this.context.closePath();
    ...
    this.context.fillStyle = bandTextColor;
    this.context.textAlign = "center";
    this.context.textBaseline = "middle";
  ...
  animateWheel() {
    ...
    if (this.animationEasing) {
      ...
      this.easingProgress = Math.min(
        this.easingProgress + cappedDelta / rampUpDuration,
        1
      );
    ...
    const rotationThisFrame =
      (this.animationSpeed * speedMultiplier * cappedDelta) / 1000;
  ...
  drawDragPreviewInRotatedContext() {
    ...
    if (!item) {
      console.warn(
        "[drawDragPreviewInRotatedContext] No draggedItem in dragState"
      );
      return;
    }
    ...
    this.wheelStructure.items.forEach((item) => {
      ...
      switch (item.dependencyType || "finish_to_start") {
        case "finish_to_start":
          ...
          startRadius =
            (predecessorPos.startRadius + predecessorPos.endRadius) / 2;
      ...
      const centerToMidDist = Math.sqrt(
        (midX - this.center.x) ** 2 + (midY - this.center.y) ** 2
      );
      const offsetFactor = this.size / 20; // Curve depth
      const controlX =
        midX + ((midX - this.center.x) / centerToMidDist) * offsetFactor;
      const controlY =
        midY + ((midY - this.center.y) / centerToMidDist) * offsetFactor;
  ...
  drawStaticElements() {
    ...
    // Add subtle border - darker on dark themes for visibility
    this.context.strokeStyle = this.isDarkTheme ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)";
    this.context.lineWidth = 1.5;
    this.context.stroke();
    this.context.closePath();

    if (this.hoveredItem) {
      // Show hovered activity info with improved layout
      const ring = this.wheelStructure.rings.find(
        (r) => r.id === this.hoveredItem.ringId
      );
      const label = this.hoveredItem.labelId
        ? this.wheelStructure.labels.find(
            (l) => l.id === this.hoveredItem.labelId
          )
        : null;
      ...
      const maxWidth = this.minRadius * 1.5; // Available width for text
      const baseSize = this.size / 70; // Base font size
      let currentY = this.center.y;

      // Calculate all elements first to center vertically
      const elements = [];

      // 1. Ring name (top, small and subtle)
      const subtleColor = "#9699a6";
      const mainTextColor = "#323338";
      const secondaryTextColor = "#676879";
      ...
      if (ring) {
        elements.push({
          ...
          color: subtleColor,
          spacing: baseSize * 0.3,
        });
      }

      // 2. Status badge (if exists)
      if (label && label.visible) {
        elements.push({
          type: "badge",
          label: label,
          fontSize: baseSize * 0.85,
          spacing: baseSize * 0.5,
        });
      }

      // 3. Item name (main focus, larger and bold)
      const nameFontSize = baseSize * 1.3;
      this.context.font = `700 ${nameFontSize}px Arial, sans-serif`;
      const nameLines = this.wrapText(this.hoveredItem.name, maxWidth);
      if (nameLines.length > 2) {
        nameLines[1] = nameLines[1].substring(0, 20) + "...";
        nameLines.length = 2;
      }
      ...
      // 4. Date range (bottom, small)
      const startDate = new Date(this.hoveredItem.startDate);
      const endDate = new Date(this.hoveredItem.endDate);
      const sameDay = this.hoveredItem.startDate === this.hoveredItem.endDate;

      let dateText;
      if (sameDay) {
        dateText = startDate.toLocaleDateString("sv-SE", {
          day: "numeric",
          month: "short",
          year: "numeric",
        });
        ...
        if (sameMonth) {
          dateText = `${startDate.getDate()}-${endDate.getDate()} ${startDate.toLocaleDateString(
            "sv-SE",
      ...
      elements.forEach((el, idx) => {
        if (el.type === "name") {
          totalHeight += el.lines.length * el.fontSize * 1.2;
        } else if (el.type === "badge") {
          totalHeight += el.fontSize * 1.8;
        } else {
          totalHeight += el.fontSize * 1.1;
        }
        if (idx < elements.length - 1) {
          totalHeight += el.spacing;
        }
      });

      // Start from top
      currentY = this.center.y - totalHeight / 2;

      // Draw each element
      elements.forEach((el) => {
        if (el.type === "ring" || el.type === "date") {
          this.context.fillStyle = el.color;
          this.context.font = `500 ${el.fontSize}px Arial, sans-serif`;
          currentY += el.fontSize * 0.6;
          this.context.fillText(el.text, this.center.x, currentY);
          currentY += el.fontSize * 0.5 + el.spacing;
        } else if (el.type === "name") {
          this.context.fillStyle = el.color;
          this.context.font = `700 ${el.fontSize}px Arial, sans-serif`;
          el.lines.forEach((line, idx) => {
            currentY += el.fontSize * 0.6;
            this.context.fillText(line, this.center.x, currentY);
            currentY += el.fontSize * 0.6;
          });
          currentY += el.spacing;
        } else if (el.type === "badge") {
          const badgePadding = el.fontSize * 0.5;
          this.context.font = `600 ${el.fontSize}px Arial, sans-serif`;
          const badgeTextWidth = this.context.measureText(el.label.name).width;
          const badgeWidth = badgeTextWidth + badgePadding * 2;
          const badgeHeight = el.fontSize + badgePadding;

          currentY += badgeHeight * 0.5;

          // Draw badge background
          this.context.fillStyle = el.label.color || "#94A3B8";
          const radius = badgeHeight / 2;
          this.context.beginPath();
          this.context.roundRect(
            this.center.x - badgeWidth / 2,
          ...
          // Draw badge text
          this.context.fillStyle = this.getContrastColor(
            el.label.color || "#94A3B8"
          );
          this.context.fillText(el.label.name, this.center.x, currentY);

          currentY += badgeHeight * 0.5 + el.spacing;
        }
      });
    } else {
      // Draw year or filtered period text in center (bold, large)
      // Center is always white background, so use dark text
      this.context.fillStyle = "#323338";
      this.context.textAlign = "center";
      this.context.textBaseline = "middle";

      let centerText = String(this.year);
      ...
      // Split text by space for line break (e.g., "December\n2025")
      const textParts = String(centerText).split(' ');
      if (textParts.length === 2) {
        // Two-line text (month + year)
        const lineHeight = fontSize * 1.1;
        this.context.fillText(
          textParts[0],
          this.center.x,
          this.center.y - lineHeight / 2,
          this.size
        );
        this.context.fillText(
          textParts[1],
        ...
        // Single line text (year only or Q1/Q2/etc)
        this.context.fillText(
          centerText,
          this.center.x,
          this.center.y,
  ...
  drawLabelIndicator(item, startRadius, width, startAngle, endAngle) {
    // Labels are NOT shown on the wheel items themselves
    // They are only shown in the center when hovering
    // This function is kept for compatibility but does nothing
    return;
  ...
  clusterItemsByWeek(items, draggedItemId = null) {
    ...
    const clusteredShortItems = Array.from(weekGroups.values()).map(
      (cluster) => {
  ...
  drawRotatingElements() {
    ...
    this.context.save();
    // Use dark divider lines on dark themes, white on light themes
    this.context.strokeStyle = this.isDarkTheme ? "rgba(255, 255, 255, 0.15)" : "#FFFFFF";
    ...
    if (
      ...
      if (visibleRings.length > 0) {
        ...
        if (totalSpaceNeeded > availableSpace) {
          console.warn(
            `Too many outer rings! Need ${totalSpaceNeeded.toFixed(
              0
            )}px but only ${availableSpace.toFixed(
              0
            )}px available. Some rings will be skipped.`
          );
        }
        ...
        visibleRings.forEach((ring, ringIndex) => {
          ...
          if (ringStartRadius < 0) {
            console.warn(
              `Ring ${
                ring.name
              } would have negative radius (${ringStartRadius.toFixed(
                2
              )}). Skipping rendering.`
            );
          ...
          // Filter items by zoom date range if zoomed
          if (this.zoomedMonth !== null || this.zoomedQuarter !== null) {
            const { startDate, endDate } = this.getDateRangeForZoom();
            ringItems = ringItems.filter((item) => {
              const itemStart = new Date(item.startDate);
              const itemEnd = item.endDate ? new Date(item.endDate) : itemStart;
              // Check if item overlaps with the zoomed date range
              return this.dateRangesOverlap(
                itemStart,
                itemEnd,
                startDate,
          ...
          // On dark themes: use white/light background for better visibility
          // On dark themes: use off-white background for better visibility without harsh edges
          // On light themes: use subtle tinted background
          if (this.isDarkTheme) {
            this.context.fillStyle = "#f0f0f0"; // Slightly off-white to reduce contrast
            this.context.globalAlpha = 1.0; // Fully opaque
          } else {
          ...
          ringItems.forEach((item) => {
            ...
            if (pendingData) {
              ...
              if (age < 300) {
                ...
                const datesMatch =
                  item.startDate === pendingData.item.startDate &&
            ...
            let itemEndDate = itemToRender.endDate
              ? new Date(itemToRender.endDate)
              : new Date(itemToRender.startDate);

            // Check if this is the highlighted item (for cross-year display)
            // Use string comparison to handle different ID types
            const isHighlighted =
              this.highlightedItem &&
              String(this.highlightedItem.id) === String(itemToRender.id);

            // For highlighted items, check for _originalStartDate/_originalEndDate (set by App.jsx for cross-year items)
            // These contain the unclamped dates when item spans year boundaries
            if (isHighlighted) {
              const origStart =
                itemToRender._originalStartDate ||
                this.highlightedItem?.startDate;
              const origEnd =
                itemToRender._originalEndDate || this.highlightedItem?.endDate;
              if (origStart) {
                itemStartDate = new Date(origStart);
              }
              if (origEnd) {
                itemEndDate = new Date(origEnd);
              }
            }

            // Store original dates for potential cross-year rendering
            const originalStartDate = new Date(itemStartDate);
            const originalEndDate = new Date(itemEndDate);

            // Track if item extends beyond current year
            const extendsBefore = originalStartDate < minDate;
            const extendsAfter = originalEndDate > maxDate;
            const isCrossYear = extendsBefore || extendsAfter;

            // VIEWPORT CULLING: Skip items outside the current date range (year or zoom)
            // BUT don't skip highlighted items that cross years
            if (
              !isHighlighted &&
              (itemEndDate < minDate || itemStartDate > maxDate)
            )
              return;

            // Clip item dates to visible boundaries (for non-highlighted items)
            if (!isHighlighted) {
              if (itemStartDate < minDate) itemStartDate = minDate;
              if (itemEndDate > maxDate) itemEndDate = maxDate;
            } else {
              // For highlighted items, clamp to reasonable bounds but keep cross-year info
              // We'll draw the overflow portions separately
              if (itemStartDate < minDate) itemStartDate = minDate;
              if (itemEndDate > maxDate) itemEndDate = maxDate;
            }
            ...
            this.setCircleSectionHTML({
              ...
              highlight: isHovered || isHighlighted,
              renderDecision: renderDecision, // Pass the decision to renderer
            });

            // Draw cross-year overflow indicators for highlighted items
            if (isHighlighted && isCrossYear) {
              this.context.save();
              this.context.setLineDash([5, 5]); // Dashed line
              this.context.globalAlpha = 0.5;

              if (extendsBefore) {
                // Draw indicator extending backwards (before Jan 1)
                // Calculate how much extends before (in the previous year)
                const daysBeforeYearStart = Math.ceil(
                  (minDate - originalStartDate) / (1000 * 60 * 60 * 24)
                );
                const overflowAngle = Math.min(
                  (daysBeforeYearStart / 365) * 360,
                  60
                ); // Cap at ~2 months visual
                const overflowStartAngle = this.initAngle - overflowAngle;
                const overflowEndAngle = this.initAngle;

                this.context.beginPath();
                this.context.arc(
                  this.center.x,
                  this.center.y,
                  itemStartRadius + itemWidth / 2,
                  this.toRadians(overflowStartAngle),
                  this.toRadians(overflowEndAngle)
                );
                this.context.strokeStyle = itemColor;
                this.context.lineWidth = itemWidth * 0.6;
                this.context.stroke();

                // Add clickable region for the overflow portion
                // Normalize angles to 0-2π range for hit detection
                let normalizedStart = this.toRadians(overflowStartAngle);
                let normalizedEnd = this.toRadians(overflowEndAngle);
                ...
                this.clickableItems.push({
                  ...
                  isOverflow: true,
                  overflowYear: this.year - 1,
                });

                // Draw "previous year" indicator
                const indicatorPos = this.moveToAngle(
                  itemStartRadius + itemWidth / 2,
                  this.toRadians(overflowStartAngle + 5)
                );
                this.context.setLineDash([]);
                this.context.fillStyle = itemColor;
                this.context.font = `bold ${this.size / 60}px Arial`;
                this.context.textAlign = "center";
                this.context.textBaseline = "middle";
                this.context.fillText(
                  `← ${this.year - 1}`,
              ...
              if (extendsAfter) {
                // Draw indicator extending forwards (after Dec 31)
                // Calculate how much extends after (into the next year)
                const daysAfterYearEnd = Math.ceil(
                  (originalEndDate - maxDate) / (1000 * 60 * 60 * 24)
                );
                const overflowAngle = Math.min(
                  (daysAfterYearEnd / 365) * 360,
                  60
                ); // Cap at ~2 months visual
                const overflowStartAngle = this.initAngle + 360;
                const overflowEndAngle = this.initAngle + 360 + overflowAngle;
                ...
                // Add clickable region for the overflow portion
                // Normalize angles to 0-2π range for hit detection
                let normalizedStartAfter = this.toRadians(overflowStartAngle);
                let normalizedEndAfter = this.toRadians(overflowEndAngle);
                while (normalizedStartAfter < 0)
                  normalizedStartAfter += Math.PI * 2;
                while (normalizedEndAfter < 0)
                  normalizedEndAfter += Math.PI * 2;
                while (normalizedStartAfter >= Math.PI * 2)
                  normalizedStartAfter -= Math.PI * 2;
                while (normalizedEndAfter >= Math.PI * 2)
                  normalizedEndAfter -= Math.PI * 2;
                ...
                this.clickableItems.push({
                  ...
                  startRadius: itemStartRadius,
                  endRadius: itemStartRadius + itemWidth,
                  startAngle: normalizedStartAfter,
                  endAngle: normalizedEndAfter,
                  isOverflow: true,
                  overflowYear: this.year + 1,
                });

                // Draw "next year" indicator
                const indicatorPos = this.moveToAngle(
                  itemStartRadius + itemWidth / 2,
                  this.toRadians(overflowEndAngle - 5)
                );
                this.context.setLineDash([]);
                this.context.fillStyle = itemColor;
                this.context.font = `bold ${this.size / 60}px Arial`;
            ...
            const isStaleData =
              !pendingData && this.pendingItemUpdates.has(item.id);
            if (!isStaleData) {
              this.clickableItems.push({
    ...
    if (totalSpaceNeeded > this.maxRadius) {
      ...
      console.error(
        `   Outer rings: ${visibleRings.length} × ${outerRingHeight.toFixed(
          0
        )}px = ${(visibleRings.length * outerRingHeight).toFixed(0)}px`
      );
      ...
      this.context.fillText(
        `Ta bort några ringar eller dölj dem`,
        this.center.x,
        this.center.y + 10
      );
      this.context.fillText(
        `för att få hjulet att visas korrekt`,
        this.center.x,
        this.center.y + 30
      );
    ...
    for (let i = 0; i < numberOfEvents; i++) {
      ...
      if (this.zoomedMonth !== null || this.zoomedQuarter !== null) {
        ...
        ringItems = ringItems.filter((item) => {
          ...
          const itemEnd = item.endDate ? new Date(item.endDate) : itemStart;
          // Check if item overlaps with the zoomed date range
          return this.dateRangesOverlap(itemStart, itemEnd, startDate, endDate);
        });
      }
      ...
      if (eventRadius < 0 || ringContentHeight < 0) {
        console.warn(
          `Inner ring ${
            ring.name
          } has invalid dimensions (radius: ${eventRadius.toFixed(
            2
          )}, height: ${ringContentHeight.toFixed(2)}). Skipping.`
        );
      ...
      // On light themes: use subtle tinted background
      if (this.isDarkTheme) {
        this.context.fillStyle = "#f0f0f0"; // Slightly off-white to reduce contrast with outer rings
        this.context.globalAlpha = 1.0; // Fully opaque
      } else {
        this.context.fillStyle = this.getLightBackgroundColor(templateColor);
        this.context.globalAlpha = ringItems.length === 0 ? 0.3 : 0.15; // More subtle for rings with items
        ...
        ringItems.forEach((item) => {
          ...
          if (isHighlighted && isCrossYear) {
            ...
            if (extendsBefore) {
              ...
              // Add clickable region for the overflow portion
              // Normalize angles to 0-2π range for hit detection
              let normalizedStartInner = this.toRadians(overflowStartAngle);
              let normalizedEndInner = this.toRadians(overflowEndAngle);
              while (normalizedStartInner < 0)
                normalizedStartInner += Math.PI * 2;
              while (normalizedEndInner < 0) normalizedEndInner += Math.PI * 2;

              this.clickableItems.push({
                itemId: itemToRender.id,
                startRadius: itemStartRadius,
                endRadius: itemStartRadius + itemWidth,
                startAngle: normalizedStartInner,
                endAngle: normalizedEndInner,
                isOverflow: true,
                overflowYear: this.year - 1,
              });
            ...
            if (extendsAfter) {
              ...
              // Add clickable region for the overflow portion
              // Normalize angles to 0-2π range for hit detection
              let normalizedStartInnerAfter =
                this.toRadians(overflowStartAngle);
              let normalizedEndInnerAfter = this.toRadians(overflowEndAngle);
              while (normalizedStartInnerAfter < 0)
                normalizedStartInnerAfter += Math.PI * 2;
              while (normalizedEndInnerAfter < 0)
                normalizedEndInnerAfter += Math.PI * 2;
              while (normalizedStartInnerAfter >= Math.PI * 2)
                normalizedStartInnerAfter -= Math.PI * 2;
              while (normalizedEndInnerAfter >= Math.PI * 2)
                normalizedEndInnerAfter -= Math.PI * 2;
              ...
              this.clickableItems.push({
                ...
                startRadius: itemStartRadius,
                endRadius: itemStartRadius + itemWidth,
                startAngle: normalizedStartInnerAfter,
                endAngle: normalizedEndInnerAfter,
                isOverflow: true,
                overflowYear: this.year + 1,
              });
    ...
    if (
      this.shouldDrawYearBoundaryHighlight(this.zoomStartDate, this.zoomEndDate)
    ) {
  ...
  drawYearBoundaryHighlight(visibleStartDate, visibleEndDate) {
    ...
    const startDate =
      visibleStartDate instanceof Date
        ? visibleStartDate
        : new Date(parseInt(this.year, 10) || new Date().getFullYear(), 0, 1);
    const endDate =
      visibleEndDate instanceof Date
        ? visibleEndDate
    ...
    const lineInnerRadius = Math.max(
      this.minRadius,
      innerRadius - this.size / 300
    );
    const lineOuterRadius = outerRadius + this.size / 300; // Just slightly beyond activities
    const lineStartX =
      this.center.x + Math.cos(boundaryAngle) * lineInnerRadius;
    const lineStartY =
      this.center.y + Math.sin(boundaryAngle) * lineInnerRadius;
    ...
    const januaryGradient = this.context.createLinearGradient(
      boundaryMidX,
      boundaryMidY, // Start at boundary
      januaryMidX,
      januaryMidY // End in January
    );
